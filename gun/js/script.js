...
class AgeVerification {
    constructor() {
        this.ageModal = document.getElementById('ageVerification');
        this.loginModal = document.getElementById('loginRequired');
        this.isAgeVerified = localStorage.getItem('ageVerified') === 'true';
        this.isUserLoggedIn = localStorage.getItem('usuario_id') !== null;
        
        this.init();
    }
    
    init() {
        // Mostrar verificação de idade se não estiver verificada
        if (!this.isAgeVerified) {
            this.showAgeVerification();
        } else if (!this.isUserLoggedIn) {
            // Se idade verificada mas não logado, mostrar modal de login
            this.showLoginRequired();
        } else {
            // Se idade verificada e logado, mostrar conteúdo normal
    ...
    setupEventListeners() {
        ...
        // Prevenir acesso via F12/DevTools
        ...
        this.ageModal.innerHTML = `
            <div class="age-verification-content">
                <div class="age-verification-header">
                    <img src="Imagens/Logo.png" alt="Sport Gun Imports" class="age-verification-logo">
                    <h1>ACESSO NEGADO</h1>
                </div>
                ...
                <div class="age-verification-body">
                    ...
                    <h2>VOCÊ PRECISA TER 18 ANOS OU MAIS</h2>
                    <p style="color: var(--text-secondary); margin-bottom: 2rem;">
                        De acordo com a legislação brasileira, o acesso a este conteúdo é restrito a maiores de 18 anos.
                    </p>
                </div>
                
                <div class="age-verification-footer">
                    <p>Volte quando completar 18 anos de idade.</p>
                </div>
            </div>
        `;
    ...
    initializeSiteComponents() {
        ...
        // Atualizar header
        if (typeof updateHeaderWithCart !== 'undefined') {
            updateHeaderWithCart();
        }
        ...
        // Carregar notícias
        if (typeof loadNews !== 'undefined') {
            loadNews();
        }
        
        // Atualizar botões dos produtos
        if (typeof atualizarBotoesProdutos !== 'undefined') {
            atualizarBotoesProdutos();
        }
    }
}
...
async function fetchRSSNews(rssUrl, sourceName) {
    try {
        ...
        if (data.status === 'ok' && data.items && data.items.length > 0) {
            console.log(`✅ ${data.items.length} itens encontrados em ${source.name}`);
...
class ShoppingCart {
    ...
    setupCartListeners() {
        // Adicionar produtos ao carrinho
        document.addEventListener('click', (e) => {
            if (e.target.closest('.btn--primary') && e.target.textContent === 'Comprar') {
                const productCard = e.target.closest('.product__card');
                if (productCard) {
                    this.addToCart(productCard);
                }
    ...
    checkout() {
        ...
        mensagem += `📍 *ENTREGA:*\n`;
        mensagem += `[Cliente informará endereço]\n\n`;
        mensagem += `_Pedido gerado via site em ${new Date().toLocaleDateString('pt-BR')}_`;
...
class WhatsAppIntegration {
    ...
    setupAgendamentoListeners() {
        // Adicionar botão de agendamento nos produtos
        document.addEventListener('click', (e) => {
            if (e.target.classList.contains('btn-agendar') || e.target.closest('.btn-agendar')) {
                const productCard = e.target.closest('.product__card');
                if (productCard) {
                    this.openAgendamentoModal(productCard);
                }
    ...
    openAgendamentoModal(productCard = null) {
        const productName = productCard ? productCard.querySelector('h4').textContent : 'Produto/Serviço';
        const productPrice = productCard ? productCard.querySelector('.price').textContent : '';
                ...
                <div class="agendamento-body">
                    <form id="agendamentoForm" class="agendamento-form">
                        <div class="form-group">
                            <label for="clienteNome"><i class="fas fa-user"></i> Nome Completo</label>
                            <input type="text" id="clienteNome" required 
                                   value="${localStorage.getItem('usuario_nome') || ''}">
                        </div>
    ...
    enviarAgendamentoWhatsApp() {
        ...
        // Criar mensagem para WhatsApp
        const mensagem = this.criarMensagemWhatsApp(agendamento);
        const urlWhatsApp = `https://wa.me/${this.phoneNumber}?text=${encodeURIComponent(mensagem)}`;
    ...
    validarAgendamento(formData) {
        if (!formData.nome || !formData.telefone || !formData.produto || !formData.data || !formData.horario) {
            this.mostrarNotificacao('Erro', 'Preencha todos os campos obrigatórios.', 'error');
            return false;
        }
...
❌ *CANCELAR* - Para cancelar o agendamento`;
    ...
    enviarNotificacaoAutomatica(agendamento) {
        ...
        setTimeout(() => {
            this.mostrarNotificacao(
                'Lembrete de Agendamento',
                `Não se esqueça do seu agendamento para ${agendamento.produto} no dia ${new Date(agendamento.data).toLocaleDateString('pt-BR')} às ${agendamento.horario}.`,
                'warning'
            );
        }, 60000); // 1 minuto
    ...
    renderAgendamentosList() {
        ...
        agendamentosContainer.innerHTML = this.agendamentos.map(agendamento => `
            <div class="agendamento-item">
                <div class="agendamento-info">
                    <div class="agendamento-details">
                        <h4>${agendamento.produto}</h4>
                        <p><i class="fas fa-user"></i> ${agendamento.nome}</p>
                        <p><i class="fas fa-calendar"></i> ${new Date(agendamento.data).toLocaleDateString('pt-BR')} às ${agendamento.horario}</p>
...
Agradecemos pela preferência! 🎯`;
        ...
        const urlWhatsApp = `https://wa.me/${agendamento.telefone.replace(/\D/g, '')}?text=${encodeURIComponent(mensagem)}`;
        window.open(urlWhatsApp, '_blank');
    }
}
...
function updateHeaderWithCart() {
    ...
    if (usuarioId) {
        ...
        // Atualizar link do menu para perfil
        if (loginMenuLink) {
            loginMenuLink.innerHTML = '<i class="fas fa-user-circle"></i> Minha Conta';
            loginMenuLink.href = '#agendamentos';
        }
    } else {
        navActions.innerHTML = `
...
function logout() {
    ...
    // Se idade foi verificada, mostrar modal de login obrigatório
    if (localStorage.getItem('ageVerified') === 'true') {
        const loginModal = document.getElementById('loginRequired');
        if (loginModal) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Inicializar verificação de idade (sempre primeiro)
    ageVerification = new AgeVerification();
    
    // Observar mudanças no login para atualizar modais
    observeLoginChanges();
});

// Observar mudanças no estado de login
function observeLoginChanges() {
    // Verificar periodicamente se o usuário fez login
    setInterval(() => {
        const isUserLoggedIn = localStorage.getItem('usuario_id') !== null;
        const loginModal = document.getElementById('loginRequired');
        
        if (isUserLoggedIn && loginModal && loginModal.style.display === 'flex') {
            // Usuário fez login, esconder modal e mostrar conteúdo
            loginModal.style.display = 'none';
            document.body.style.overflow = 'auto';
            
            // Inicializar componentes do site
            if (ageVerification) {
                ageVerification.initializeSiteComponents();
            }
        }
    }, 1000);
}
